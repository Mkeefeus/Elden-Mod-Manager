...
const ModTable = () => {
  ...
  const rows = mods.map((mod, index) => {
    ...
    return (
      <Table.Tr
        key={mod.uuid}
        style={{ opacity: mod.enabled ? 1 : 0.4, transition: 'opacity 0.15s ease' }}
      >
        <Table.Td>
          <Center>
            <Checkbox
        ...
        <Table.Td>
          <Center>
            {depCount > 0 ? (
              <Badge size="sm" variant="light">{depCount}</Badge>
            ) : (
        ...
        <Table.Td>
          <Center>
            {mod.dllFile
              ? <Text size="sm" c="green" fw={600}>✓</Text>
              : <Text size="sm" c="dimmed">—</Text>}
          </Center>
        </Table.Td>
  ...
  return (
    <Table withRowBorders highlightOnHover layout="fixed" horizontalSpacing="xs" verticalSpacing="xs" style={{ width: '100%', minWidth: 520 }}>
      <Table.Thead>
        <Table.Tr>
          ...
          <Table.Th c="dimmed" style={{ width: 90 }}>More</Table.Th>
        </Table.Tr>
      </Table.Thead>
